...
Реализованы D1 storage, application-сценарии и grammY-обработчики: доступ, оба способа создания без дубликатов, история, детали, редактирование, удаление с подтверждением и статистика с навигацией по месяцам и календарём для произвольного периода. Все даты календаря передаются в кнопках, без серверного состояния. Интеграционные тесты используют временную локальную D1 и имитацию Bot API; реальная Cloudflare D1 и Telegram пока не проверялись.
